...
- 日期：2026-08-15
- 状态：已评审（2026-08-15 拷问评审通过，结论见 §10）；UI 与部分行为已被 [2026-08-16-manager-redesign.md](2026-08-16-manager-redesign.md) 增量覆盖——读本 spec 时以该文件声明的覆盖项为准
- 修订：2026-08-17，§2 非目标中「接管新标签页」的否决被撤销、「搜索功能」的范围被收窄，见 [2026-08-17-newtab-override.md](2026-08-17-newtab-override.md) 与 [2026-08-17-omnibox-keyword.md](2026-08-17-omnibox-keyword.md)，理由记录于 §10
...
- **管理页内的搜索**：不做 tab 过滤搜索框，不做历史搜索（已在评审中移除）。2026-08-17 收窄：本条约束的是**管理页内**的搜索 UI；地址栏关键字搜索自有数据不在此列，见 [2026-08-17-omnibox-keyword.md](2026-08-17-omnibox-keyword.md) §2
- **网页搜索**：不做任何形式的网页搜索——不设页内搜索框、不覆写默认搜索引擎、无匹配时不降级到搜索引擎。它与 tab 管理无关，会把扩展变成多用途
- **内嵌历史面板**：不做最近记录列表 / 搜索历史 /「打开完整历史」按钮，历史仅保留一个直达入口
- ~~**接管新标签页**~~：**2026-08-17 撤销此否决**，改由 [2026-08-17-newtab-override.md](2026-08-17-newtab-override.md) 实现。原否决理由（`chrome_url_overrides` 无法动态开关、无法把用户导航回原生新标签页）在事实上依然成立，但被重新评估为可接受代价而非阻塞：接管不可逆这一点以商店描述与 README 的显式披露应对；「无法动态开关」原本假设需要一个「是否接管」开关，而该开关本身已被判定为伪需求
...
- **语音操作**：不做语音控制。tab 管理是视觉定位型高频微操（扫视列表→点击/拖拽，单动作 <1 秒），语音指令说完加识别延迟即已落后，且用户操作 tab 时手必然在键鼠上，语音的"解放双手"前提不成立；自然语言指令还会从后门带回已否决的搜索。无障碍诉求以键盘可达性 + ARIA 语义满足，交给系统级语音控制工具（AI 批量操作的观察项见 `ROADMAP.md`）
...
### 2026-08-17 新标签页与地址栏关键字评审

| 决策点 | 结论 |
|---|---|
| 接管新标签页 | 撤销 §2 的否决。接管不可逆是可接受代价（以商店描述与 README 显式披露应对），原否决所设想的「是否接管」开关被判定为伪需求 |
| 新标签页内容 | 渲染与管理页**完全相同**的界面，不做简化版——两套界面即两份维护成本与两处会漂移的行为 |
| 新标签页与单例的关系 | URL 路径分开、UI 组件共享。`chrome_url_overrides` 指向 `src/newtab/index.html` 而非管理页 URL，`findManagerTab` 因而天然不匹配，单例逻辑零改动；该约束由护栏测试固化 |
| 自有页面隐身 | 判定基准为扩展根前缀而非枚举 URL 清单。前者精确等于「本扩展全部页面」，新增页面自动覆盖；枚举清单的漏登记是静默失效 |
| 关闭窗口语义 | 判定基准由管理页精确 URL 改为扩展根前缀，规则统一为「自有页面不被关闭类操作波及」。否决了用 `tabs.getCurrent()` 只保留当前页的方案——它引入与「自有页面」并列的第二概念 |
| 搜索的边界 | 管理页内搜索与网页搜索维持否决；地址栏关键字只搜自有数据、无匹配时不降级到网页搜索，故不受该否决约束 |
| 页内搜索框 | 否决。⌘T 后光标已在地址栏，与之功能重复；且它是唯一会把「网页搜索」这一第二用途引入扩展的东西 |
